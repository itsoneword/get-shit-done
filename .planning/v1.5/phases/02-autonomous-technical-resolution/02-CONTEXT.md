...
- Reuse `deep-research` (fan-out + adversarial-verify) for the heavy path and the existing micro-research shape for the light path; **do not rebuild research capability**. [STRONG — user's anti-redundancy ethos; verified triple-coverage of research already exists]
