...
> **已经有现成代码库？** 先运行 `/gsd2:map-codebase`。它会并行拉起多个代理分析你的技术栈、架构、约定和风险点。之后 `/gsd2:new-project` 就会真正“理解”你的代码库，提问会聚焦在你打算新增的部分，规划时也会自动加载你的现有模式。
...
如果一切通过，就进入下一步；如果哪里坏了，你不需要手动 debug，只要重新运行 `/gsd2:execute-phase`，执行它自动生成的修复计划即可。
...
```
/gsd2:discuss-phase 2
/gsd2:plan-phase 2
/gsd2:execute-phase 2
/gsd2:verify-work 2
...
...
如果你希望在讨论阶段更快收集信息，可以用 `/gsd2:discuss-phase <n> --batch`，一次回答一小组问题，而不是逐个问答。

每个阶段都会得到你的输入（discuss）、充分研究（plan）、干净执行（execute）和人工验证（verify）。上下文始终保持新鲜，质量也能持续稳定。

当所有阶段完成后，`/gsd2:complete-milestone` 会归档当前里程碑并打 release tag。

接着用 `/gsd2:new-milestone` 开启下一个版本。它和 `new-project` 流程相同，只是面向你现有的代码库。你描述下一步想构建什么，系统研究领域、梳理需求，再产出新的路线图。每个里程碑都是一个干净周期：定义 → 构建 → 发布。
...
| 命令 | 作用 |
|------|------|
| `/gsd2:new-project [--auto]` | 完整初始化：提问 → 研究 → 需求 → 路线图 |
| `/gsd2:discuss-phase [N] [--auto]` | 在规划前收集实现决策 |
| `/gsd2:plan-phase [N] [--auto]` | 为某个阶段执行研究 + 规划 + 验证 |
| `/gsd2:execute-phase <N>` | 以并行 wave 执行全部计划，完成后验证 |
| `/gsd2:verify-work [N]` | 人工用户验收测试 ¹ |
| `/gsd2:audit-milestone` | 验证里程碑是否达到完成定义 |
| `/gsd2:complete-milestone` | 归档里程碑并打 release tag |
| `/gsd2:new-milestone [name]` | 开始下一个版本：提问 → 研究 → 需求 → 路线图 |

### 导航

| 命令 | 作用 |
|------|------|
| `/gsd2:progress` | 我现在在哪？下一步是什么？ |
| `/gsd2:help` | 显示全部命令和使用指南 |
| `/gsd2:update` | 更新 GSD，并预览变更日志 |
| `/gsd2:join-discord` | 加入 GSD Discord 社区 |

### Brownfield

| 命令 | 作用 |
|------|------|
| `/gsd2:map-codebase` | 在 `new-project` 前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|------|------|
| `/gsd2:add-phase` | 在路线图末尾追加 phase |
| `/gsd2:insert-phase [N]` | 在 phase 之间插入紧急工作 |
| `/gsd2:remove-phase [N]` | 删除未来 phase，并重编号 |
| `/gsd2:list-phase-assumptions [N]` | 在规划前查看 Claude 打算采用的方案 |
| `/gsd2:plan-milestone-gaps` | 为 audit 发现的缺口创建 phase |

### 会话

| 命令 | 作用 |
|------|------|
| `/gsd2:pause-work` | 在中途暂停时创建交接上下文 |
| `/gsd2:resume-work` | 从上一次会话恢复 |

### 工具

| 命令 | 作用 |
|------|------|
| `/gsd2:settings` | 配置模型 profile 和工作流代理 |
| `/gsd2:set-profile <profile>` | 切换模型 profile（quality / balanced / budget） |
| `/gsd2:add-todo [desc]` | 记录一个待办想法 |
| `/gsd2:check-todos` | 查看待办列表 |
| `/gsd2:debug [desc]` | 使用持久状态进行系统化调试 |
| `/gsd2:quick [--full] [--discuss]` | 以 GSD 保障执行临时任务（`--full` 增加计划检查和验证，`--discuss` 先补上下文） |
| `/gsd2:health [--repair]` | 校验 `.planning/` 目录完整性，带 `--repair` 时自动修复 |
...
GSD 将项目设置保存在 `.planning/config.json`。你可以在 `/gsd2:new-project` 时配置，也可以稍后通过 `/gsd2:settings` 修改。完整的配置 schema、工作流开关、git branching 选项以及各代理的模型分配，请查看[用户指南](docs/USER-GUIDE.md#configuration-reference)。
...
切换方式：
```
/gsd2:set-profile budget
```

也可以通过 `/gsd2:settings` 配置。
...
可以用 `/gsd2:settings` 开关这些项，也可以在单次命令里覆盖：
- `/gsd2:plan-phase --skip-research`
- `/gsd2:plan-phase --skip-verify`
