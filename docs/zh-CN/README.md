...
> **已有代码？** 先运行 `/gsd2:map-codebase`。它会生成并行代理分析你的技术栈、架构、约定和关注点。然后 `/gsd2:new-project` 就了解你的代码库了 —— 问题聚焦在你正在**添加**什么，规划会自动加载你的模式。
...
如果一切通过，继续。如果有东西坏了，不用手动调试 —— 只需再次运行 `/gsd2:execute-phase`，使用它创建的修复计划。
...
如果你想在讨论期间更快速地输入，使用 `/gsd2:discuss-phase <n> --batch` 一次回答一组小问题，而不是一个一个来。

每个阶段都会获得你的输入（讨论）、适当的研究（规划）、干净的执行（执行）和人工验证（验证）。上下文保持新鲜。质量保持高水平。

当所有阶段完成后，`/gsd2:complete-milestone` 归档里程碑并标记发布。

然后 `/gsd2:new-milestone` 开始下一个版本 —— 与 `new-project` 相同的流程，但针对你现有的代码库。你描述接下来想构建什么，系统研究领域，你界定需求范围，它创建新的路线图。每个里程碑是一个干净的周期：定义 → 构建 → 发布。
...
| `/gsd2:discuss-phase [N] [--auto]` | 在规划前捕获实现决策 |
| `/gsd2:plan-phase [N] [--auto]` | 阶段的研究 + 规划 + 验证 |
| `/gsd2:execute-phase <N>` | 在并行波次中执行所有计划，完成后验证 |
| `/gsd2:verify-work [N]` | 手动用户验收测试 ¹ |
| `/gsd2:audit-milestone` | 验证里程碑达到了其完成定义 |
| `/gsd2:complete-milestone` | 归档里程碑，标记发布 |
...
| 命令 | 作用 |
|---------|--------------|
| `/gsd2:progress` | 我在哪？接下来做什么？ |
| `/gsd2:help` | 显示所有命令和使用指南 |
...
| 命令 | 作用 |
|---------|--------------|
| `/gsd2:map-codebase` | 在 new-project 之前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|---------|--------------|
| `/gsd2:add-phase` | 向路线图追加阶段 |
| `/gsd2:insert-phase [N]` | 在阶段之间插入紧急工作 |
| `/gsd2:remove-phase [N]` | 删除未来阶段，重新编号 |
| `/gsd2:list-phase-assumptions [N]` | 规划前查看 Claude 的预期方法 |
| `/gsd2:plan-milestone-gaps` | 创建阶段以填补审计发现的差距 |

### 会话

| 命令 | 作用 |
|---------|--------------|
| `/gsd2:pause-work` | 阶段中途停止时创建交接 |
| `/gsd2:resume-work` | 从上次会话恢复 |

### 工具

| 命令 | 作用 |
|---------|--------------|
| `/gsd2:settings` | 配置模型配置文件和工作流代理 |
| `/gsd2:set-profile <profile>` | 切换模型配置文件（quality/balanced/budget） |
| `/gsd2:add-todo [desc]` | 捕获想法留待后用 |
| `/gsd2:check-todos` | 列出待处理事项 |
| `/gsd2:debug [desc]` | 带持久状态的系统化调试 |
| `/gsd2:quick [--full] [--discuss]` | 用 GSD 保证执行临时任务（`--full` 添加计划检查和验证，`--discuss` 先收集上下文） |
| `/gsd2:health [--repair]` | 验证 `.planning/` 目录完整性，用 `--repair` 自动修复 |
...
GSD 在 `.planning/config.json` 中存储项目设置。在 `/gsd2:new-project` 期间配置或稍后用 `/gsd2:settings` 更新。完整配置模式、工作流开关、git 分支选项和每个代理的模型分解，请参阅[用户指南](USER-GUIDE.md#配置参考)。
...
使用 `/gsd2:settings` 切换这些，或每次调用时覆盖：
