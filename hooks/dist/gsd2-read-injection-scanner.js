#!/usr/bin/env node
// gsd-hook-version: 1.5.2
// GSD2 Read Injection Scanner — PostToolUse hook (#2201)
...
//
// Config gate: hooks.read_injection_scanner (default ON — runs unless explicitly false)
//
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Config gate: default ON — run unless explicitly disabled
    const cwd = data.cwd || process.cwd();
    try {
      const configPath = path.join(cwd, '.planning', 'config.json');
      if (fs.existsSync(configPath)) {
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        if (config.hooks?.read_injection_scanner === false) process.exit(0); // disabled only by explicit false
      }
