...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /gsd2:new-project to set up.',
...
const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  ...
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/gsd2:profile-user` to generate your developer profile.',
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/gsd2:profile-user --refresh` with session data to populate.';
  ...
  const result = {
    ...
    command_name: '/gsd2:dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GSD from ${dataSource}. Run \`/gsd2:profile-user --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /gsd2:profile-user to unlock Developer Profile.';
