...
function cmdValidateHealth(cwd, options, raw) {
  ...
  const info = [];
  const repairs = [];
  // Set by Check 4b when STATE.md milestone frontmatter has drifted; consumed by
  // the resyncStateMilestone repair to rewrite it deterministically.
  let driftedMilestoneTarget = null;
  let driftedMilestoneName = null;
  ...
  // ─── Check 4b: STATE.md milestone frontmatter drift ───────────────────────
  // STATE.md `milestone:` can drift from the active milestone (it's only rewritten
  // on a STATE.md write). When it does, phasesDir() would create a stray
  // .planning/<stale>/phases tree. Flag the drift if — and only if — a different
  // real milestone can be corroborated from ROADMAP or on-disk dirs (mirrors the
  // recovery logic in core.phasesDir).
  if (fs.existsSync(statePath)) {
    const stateRaw = fs.readFileSync(statePath, 'utf-8');
    const sm = stateRaw.match(/^milestone:\s*(.+)$/m);
    const stateMilestone = sm ? sm[1].trim() : null;
    if (stateMilestone && !fs.existsSync(path.join(planningDir, stateMilestone, 'phases'))) {
      let real = null;
      let realName = null;
      try {
        const mi = getMilestoneInfo(cwd);
        if (mi && mi.version && mi.version !== stateMilestone
            && fs.existsSync(path.join(planningDir, mi.version, 'phases'))) {
          real = mi.version;
          realName = mi.name;
        }
      } catch { /* fall through */ }
      if (!real) {
        try {
          const candidates = fs.readdirSync(planningDir, { withFileTypes: true })
            .filter(e => e.isDirectory() && /^v\d+(?:\.\d+)+$/.test(e.name) && e.name !== stateMilestone)
            .filter(e => fs.existsSync(path.join(planningDir, e.name, 'phases')))
            .map(e => e.name);
          if (candidates.length === 1) real = candidates[0];
        } catch { /* fall through */ }
      }
      if (real) {
        driftedMilestoneTarget = real;
        driftedMilestoneName = realName;
        addIssue(
          'warning',
          'W010',
          `STATE.md milestone "${stateMilestone}" has no phases dir; active milestone appears to be "${real}". phase add would create a stray .planning/${stateMilestone}/`,
          'Run /gsd2:health --repair to rewrite STATE.md frontmatter to the active milestone',
          true
        );
        if (!repairs.includes('resyncStateMilestone')) repairs.push('resyncStateMilestone');
      }
    }
  }
    ...
    for (const repair of repairs) {
      try {
        switch (repair) {
          ...
          case 'resyncStateMilestone': {
            if (fs.existsSync(statePath) && driftedMilestoneTarget) {
              // Backup before rewriting frontmatter
              const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
              const backupPath = `${statePath}.bak-${ts}`;
              fs.copyFileSync(statePath, backupPath);
              repairActions.push({ action: 'backupState', success: true, path: backupPath });
              let c = fs.readFileSync(statePath, 'utf-8');
              c = c.replace(/^(milestone:\s*).+$/m, `$1${driftedMilestoneTarget}`);
              if (driftedMilestoneName) {
                c = c.replace(/^(milestone_name:\s*).+$/m, `$1${driftedMilestoneName}`);
              }
              fs.writeFileSync(statePath, c, 'utf-8');
              repairActions.push({ action: repair, success: true, path: 'STATE.md' });
            }
            break;
          }
