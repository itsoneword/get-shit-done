...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during /gsd2:discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
    ...
    case 'phase-dir': {
      ...
      output({ created: true, directory: `${relPhasesPath(cwd)}/${dirName}`, path: dirPath }, raw, dirPath);
      return;
    }
    default:
