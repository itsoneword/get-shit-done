...
- **Mutable default argument** — `def f(x=[])` shares one list across all calls; use `def f(x=None): x = x or []`
- **Late-binding closures** — loop variable captured by reference; all lambdas see the final value; bind with `default=val`
- **`is` vs `==`** — identity vs equality; works for small ints and `None`, silently wrong for strings and larger ints
- **Implicit `None` return** — a branch with no `return` returns `None` silently; callers get unexpected `None`
- **Bare `except:`** — swallows `KeyboardInterrupt`/`SystemExit`; catch `Exception` instead
- **Generator exhaustion** — iterating a generator twice; the second pass yields nothing
- **Shallow vs deep copy** — `list[:]` / `dict.copy()` don't deep-copy nested structures; use `copy.deepcopy()`
- **Integer division** — Python 2: `/` truncates; Python 3: use `//` explicitly when integer division intended
