...
Todos are captured during work sessions with /gsd2:add-todo.

---

Would you like to:

1. Continue with current phase (/gsd2:progress)
2. Add a todo now (/gsd2:add-todo)
```
...
- `/gsd2:check-todos` → show all
- `/gsd2:check-todos api` → filter to area:api only
</step>
...
- `/gsd2:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /gsd2:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/gsd2:add-phase [description from todo]`
