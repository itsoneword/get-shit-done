...
Then re-run: npx github:itsoneword/get-shit-done
`);
    process.exit(1);
  }
}
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx github:itsoneword/get-shit-done [options]\n\n  ${yellow}Options:${reset}\n    ${cyan}-g, --global${reset}              Install globally (to config directory)\n    ${cyan}-l, --local${reset}               Install locally (to current directory)\n    ${cyan}--claude${reset}                  Install for Claude Code only\n    ${cyan}--opencode${reset}                Install for OpenCode only\n    ${cyan}--gemini${reset}                  Install for Gemini only\n    ${cyan}--codex${reset}                   Install for Codex only\n    ${cyan}--copilot${reset}                 Install for Copilot only\n    ${cyan}--antigravity${reset}             Install for Antigravity only\n    ${cyan}--cursor${reset}                  Install for Cursor only\n    ${cyan}--all${reset}                     Install for all runtimes\n    ${cyan}-u, --uninstall${reset}           Uninstall GSD (remove all GSD files)\n    ${cyan}-c, --config-dir <path>${reset}   Specify custom config directory\n    ${cyan}-h, --help${reset}                Show this help message\n    ${cyan}--force-statusline${reset}        Replace existing statusline config\n\n  ${yellow}Examples:${reset}\n    ${dim}# Interactive install (prompts for runtime and location)${reset}\n    npx github:itsoneword/get-shit-done\n\n    ${dim}# Install for Claude Code globally${reset}\n    npx github:itsoneword/get-shit-done --claude --global\n\n    ${dim}# Install for Gemini globally${reset}\n    npx github:itsoneword/get-shit-done --gemini --global\n\n    ${dim}# Install for Codex globally${reset}\n    npx github:itsoneword/get-shit-done --codex --global\n\n    ${dim}# Install for Copilot globally${reset}\n    npx github:itsoneword/get-shit-done --copilot --global\n\n    ${dim}# Install for Copilot locally${reset}\n    npx github:itsoneword/get-shit-done --copilot --local\n\n    ${dim}# Install for Antigravity globally${reset}\n    npx github:itsoneword/get-shit-done --antigravity --global\n\n    ${dim}# Install for Antigravity locally${reset}\n    npx github:itsoneword/get-shit-done --antigravity --local\n\n    ${dim}# Install for Cursor globally${reset}\n    npx github:itsoneword/get-shit-done --cursor --global\n\n    ${dim}# Install for Cursor locally${reset}\n    npx github:itsoneword/get-shit-done --cursor --local\n\n    ${dim}# Install for all runtimes globally${reset}\n    npx github:itsoneword/get-shit-done --all --global\n\n    ${dim}# Install to custom config directory${reset}\n    npx github:itsoneword/get-shit-done --codex --global --config-dir ~/.codex-work\n\n    ${dim}# Install to current project only${reset}\n    npx github:itsoneword/get-shit-done --claude --local\n\n    ${dim}# Uninstall GSD from Cursor globally${reset}\n    npx github:itsoneword/get-shit-done --cursor --global --uninstall\n\n  ${yellow}Notes:${reset}\n    The --config-dir option is useful when you have multiple configurations.\n    It takes priority over CLAUDE_CONFIG_DIR / GEMINI_CONFIG_DIR / CODEX_HOME / COPILOT_CONFIG_DIR / ANTIGRAVITY_CONFIG_DIR / CURSOR_CONFIG_DIR environment variables.\n`);
  process.exit(0);
}
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all gsd: references → gsd2-)
  c = c.replace(/gsd2:/g, 'gsd2-');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all gsd: references → gsd2-)
  c = c.replace(/gsd2:/g, 'gsd2-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  ...
  // This preserves rendered "next step" commands like "/gsd2-execute-phase 17".
  return content.replace(/gsd2:/gi, 'gsd2-');
}
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/gsd2:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$gsd2-${String(commandName).toLowerCase()}`;
  });
  converted = converted.replace(/\/gsd-help\b/g, '$gsd2-help');
  return converted;
}
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /gsd2:command with /gsd2-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/gsd2:/g, '/gsd2-');
 ...
 * OpenCode expects: command/gsd2-help.md (invoked as /gsd2-help)
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/gsd2/debug/start.md -> command/gsd2-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> gsd2-help.md
...
function listCodexSkillNames(skillsDir, prefix = 'gsd2-') {
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/gsd2:/gi, 'gsd2-');
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    // OpenCode: remove command/gsd-*.md and command/gsd2-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if ((file.startsWith('gsd-') || file.startsWith('gsd2-')) && file.endsWith('.md')) {
    ...
    // Codex/Cursor: remove skills/gsd-*/SKILL.md and skills/gsd2-*/SKILL.md skill directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && (entry.name.startsWith('gsd-') || entry.name.startsWith('gsd2-'))) {
          fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
          skillCount++;
        }
      }
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if ((file.startsWith('gsd-') || file.startsWith('gsd2-')) && file.endsWith('.toml')) {
    ...
    // Copilot: remove skills/gsd-*/ and skills/gsd2-*/ directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Antigravity: remove skills/gsd-*/ and skills/gsd2-*/ directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Cursor: remove skills/gsd-*/ and skills/gsd2-*/ directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'copilot')
      ...
      : runtime === 'codex'
        ? '$gsd2-reapply-patches'
        : runtime === 'cursor'
          ? 'gsd2-reapply-patches (mention the skill name)'
          : '/gsd2:reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    // Copy commands/gsd2/*.md as command/gsd2-*.md (flatten structure)
    const gsdSrc = path.join(src, 'commands', 'gsd2');
    copyFlattenedCommands(gsdSrc, commandDir, 'gsd2', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/gsd2-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('gsd2-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/gsd2-*');
    }
  } else if (isCodex) {
    const skillsDir = path.join(targetDir, 'skills');
    const gsdSrc = path.join(src, 'commands', 'gsd2');
    copyCommandsAsCodexSkills(gsdSrc, skillsDir, 'gsd2', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
      failures.push('skills/gsd2-*');
    }
  } else if (isCopilot) {
    const skillsDir = path.join(targetDir, 'skills');
    const gsdSrc = path.join(src, 'commands', 'gsd2');
    copyCommandsAsCopilotSkills(gsdSrc, skillsDir, 'gsd2', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('gsd2-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const gsdSrc = path.join(src, 'commands', 'gsd2');
    copyCommandsAsAntigravitySkills(gsdSrc, skillsDir, 'gsd2', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('gsd2-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const gsdSrc = path.join(src, 'commands', 'gsd2');
    copyCommandsAsCursorSkills(gsdSrc, skillsDir, 'gsd2', pathPrefix, runtime);
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true) {
  ...
  if (runtime === 'opencode') command = '/gsd2-new-project';
  if (runtime === 'codex') command = '$gsd2-new-project';
  if (runtime === 'copilot') command = '/gsd2-new-project';
  if (runtime === 'antigravity') command = '/gsd2-new-project';
  if (runtime === 'cursor') command = 'gsd2-new-project (mention the skill name)';
  console.log(`
